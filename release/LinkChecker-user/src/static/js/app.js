...
const includeExternalInput = document.getElementById("include-external");
...
const scanWorkersInput = document.getElementById("scan-workers");
...
function getScanOptions() {
    return {
        include_external: includeExternalInput.checked,
        include_assets: includeAssetsInput.checked,
        timeout: clampNumber(scanTimeoutInput.value, 20, 1, 60),
        max_workers: clampNumber(scanWorkersInput.value, 12, 1, 32),
    };
}


function setScanControlsDisabled(isDisabled) {
    scanButton.disabled = isDisabled;
    includeExternalInput.disabled = isDisabled;
    includeAssetsInput.disabled = isDisabled;
    scanTimeoutInput.disabled = isDisabled;
    scanWorkersInput.disabled = isDisabled;
}


function updateLoadingStep(index) {
    loadingStepIndex = Math.min(index, loadingSteps.length - 1);
    loadingStepElement.textContent = loadingSteps[loadingStepIndex];

    loadingStepsElement.querySelectorAll("li").forEach((item) => {
        const itemIndex = Number(item.dataset.step);

        item.dataset.state = itemIndex < loadingStepIndex
            ? "done"
            : itemIndex === loadingStepIndex
                ? "active"
                : "pending";
    });
}
...
function startLoadingState(pageUrl, label = "Scanning") {
    emptySection.classList.add("hidden");
    resultsSection.classList.add("hidden");
    loadingSection.classList.remove("hidden");
    loadingUrlElement.textContent = pageUrl;
    updateLoadingStep(0);
    setAppStatus("scanning", label);

    window.clearInterval(loadingTimer);

    loadingTimer = window.setInterval(() => {
        updateLoadingStep(loadingStepIndex + 1);
    }, 1800);
}
...
function renderInlineActions(value, label) {
    if (!value) {
        return "";
    }

    const escapedValue = escapeHtml(value);
    const openAction = isClickableUrl(value)
        ? `
            <a
                class="icon-action"
                href="${escapedValue}"
...
function exportCurrentResultsToCsv() {
    ...
    const rows = filteredResults.map((result) => [
        currentSourcePage,
        result.status ?? getStatusLabel(getStatusGroup(result), result.status),
        getStatusGroup(result),
        result.http_status ?? "",
        result.url ?? "",
        result.final_url ?? "",
        formatRedirectChain(result.redirect_chain),
        ...
        issueItem.innerHTML = `
            ...
            <div class="issue-content">
                ...
                <dl class="issue-details">
                    <div>
                        <dt>Found at</dt>
                        <dd>${escapeHtml(result.source_location ?? "-")}</dd>
                    </div>
                    <div>
                        <dt>Final URL</dt>
                        <dd>${escapeHtml(result.final_url ?? "-")}</dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="issue-meta">
                ...
                ${renderInlineActions(result.url, "issue URL")}
            </div>
        `;
...
function renderTable(results) {
    ...
    filteredResults.forEach((result) => {
        ...
        const details = [
            ...
            ["Attribute", result.source_attribute ?? "-"],
...
function renderScanHistory(history) {
    ...
    history.forEach((item) => {
        ...
        const issueCount = summary.needs_action_count ??
            (item.broken ?? 0) + (item.error ?? 0);
        const healthScore = summary.health_score;
        const healthLabel = Number.isFinite(Number(healthScore))
            ? `${healthScore}% health`
            : `${item.total_links ?? 0} links`;
        ...
        historyItem.innerHTML = `
            ...
            <span class="history-counts">
                ${escapeHtml(healthLabel)} | ${escapeHtml(issueCount)} issues
            </span>
        `;
...
async function refreshCurrentScanOrHistory() {
    ...
    startLoadingState(pageUrl, "Refreshing");
    refreshHistoryButton.textContent = "Refreshing...";
...
scanForm.addEventListener("submit", async (event) => {
    ...
    formError.classList.add("hidden");
    startLoadingState(pageUrl, "Scanning");
...
document.addEventListener("click", async (event) => {
    ...
    if (!copyButton) {
        const iconCopyButton = event.target.closest(".icon-action[data-copy]");

        if (!iconCopyButton) {
            return;
        }

        await copyValueFromButton(iconCopyButton);
...
async function copyValueFromButton(copyButton) {
